#!/usr/bin/env python3
# limpiador_excel_gui.py (versión parcheada para preservar enteros largos)
import os
import sys
import subprocess
        ...
        if isinstance(raw_value, str):
            ...
            if parsed_num is not None:
                ...
                if tag == 'AS_TEXT':
                    ...
                    copy_cell_style(cell_style, tgt)
                    try:
                        tgt.number_format = '@'  # forzar formato texto en Excel
                    except Exception:
                        pass
                    return
                else:
            ...
            if parsed_date is not None:
                ...
                if not custom_fmt:
                    try:
                        tgt.number_format = 'yyyy-mm-dd hh:mm:ss'
                    except Exception:
                        pass
                else:
                    try:
                        ...
                        try:
                            if isinstance(cell_val, str):
                                ...
                                if parsed_date is not None:
                                    ...
                                    ws_v.cell(row=r_idx, column=c_idx).number_format = 'yyyy-mm-dd hh:mm:ss'
                                    continue
